...
RunCoNGA <- function(variable_features_file, tcr_datafile, gex_datafile, organism,
                     ...
                     outfile_prefix_for_qc_plots, working_directory = getwd(), conga_prefix = "conga_") {
  ...
  #Append conga_prefix so we can run Conga iteratively and record the clustering information.
  colnames(conga_dataframe) <- paste0(conga_prefix, colnames(conga_dataframe))
  seuratObj <- Seurat::AddMetaData(seuratObj, metadata = conga_dataframe[,paste0(conga_prefix,c("clusters_gex", "clusters_tcr", "is_invariant", "conga_scores"))])
}