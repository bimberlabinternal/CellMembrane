...
Q3_Normalization <- function(seuratObj, assay = "RNA", targetAssayName = "Q3"){
  ...
  counts <- as.matrix(Seurat::GetAssayData(seuratObj, assay = assay, layer = 'counts'))
...
RLE_Plot <- function(seuratObj, assay = "RNA", sampleIdentifier = "SegmentDisplayName", colorVariable){
  ...
  deviations <- as.matrix(log2(Seurat::GetAssayData(seuratObj, assay = assay, layer = 'counts')+1)) - matrixStats::rowMedians(as.matrix(log2(Seurat::GetAssayData(seuratObj, assay = assay, layer = 'counts')+1)))
