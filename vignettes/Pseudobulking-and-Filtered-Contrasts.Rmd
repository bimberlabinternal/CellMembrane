...
logger::log_info('This is the WD:')
logger::log_info(getwd())
logger::log_info(system.file(package = 'CellMembrane', mustWork = FALSE))
...
seuratObj <- suppressMessages(suppressWarnings(
    Seurat::UpdateSeuratObject(readRDS(paste0(system.file(package = 'CellMembrane', mustWork = FALSE), "/tests/testdata/seuratOutput.rds")))
))
...
metadata_tiles <- ggplot(barplot_results$dataframe) + 
  ...
  scale_fill_manual(values = rev(hcl.colors(n_distinct(pbulk$timepoint), palette = "Purples 3")), guide = guide_legend(order = 1))+
  labs(fill = "Timepoint")+
  geom_tile(aes(x = reorder(contrast_name, -abs(count)), y = 1, fill = barPlot_dataframe$positive_contrast_timepoint)) +
  geom_tile(aes(x = reorder(contrast_name, -abs(count)), y = 2, fill = barPlot_dataframe$negative_contrast_timepoint)) +
  ggnewscale::new_scale_fill() +
  
  #define a custom scale for vaccine_cohort metadata
  scale_fill_manual(values = rev(hcl.colors(n_distinct(pbulk$vaccine_cohort), palette = "Inferno")), guide = guide_legend(order = 1))+
  labs(fill = "Vaccine")+
  geom_tile(aes(x = reorder(contrast_name, -abs(count)), y = 3, fill = barPlot_dataframe$positive_contrast_vaccine_cohort)) +
  geom_tile(aes(x = reorder(contrast_name, -abs(count)), y = 4, fill = barPlot_dataframe$negative_contrast_vaccine_cohort)) +
  ggnewscale::new_scale_fill() +
  ...
  scale_y_discrete(limits = rev(c("Positive Contrast Timepoint", "Negative Contrast Timepoint","Positive Contrast Vaccine","Negative Contrast Vaccine"))) + facet_wrap(~DEG_Magnitude, scales = "free_x")
